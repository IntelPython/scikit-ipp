...
def test_rotate():
    ...
    x[1, 1] = 1
    x90 = skipp.transform.rotate(x, angle=90)
...
def test_rotate_resize():
    ...
    x45 = skipp.transform.rotate(x, 45, resize=False)
    assert x45.shape == (10, 10)

    x45 = skipp.transform.rotate(x, 45, resize=True)
...
def test_rotate_center():
    ...
    refx[2, 5] = 1
    x20 = skipp.transform.rotate(x, 20, order=0, center=(0, 0))
    assert_allclose(x20, refx, rtol=1e-06)
    x0 = skipp.transform.rotate(x20, -20, order=0, center=(0, 0))
...
def test_rotate_resize_center():
    ...
    x45 = skipp.transform.rotate(x, 45, resize=True, center=(3, 3), order=0)
    # new dimension should be d = sqrt(2 * (10/2)^2)
    assert x45.shape == (14, 14)
...
def test_rotate_resize_90():
    x90 = skipp.transform.rotate(np.zeros((470, 230), dtype=np.double), 90,
                                       resize=True)
...
def test_rotate_order_error(order):
    with pytest.raises(ValueError):
        skipp.transform.rotate(np.zeros((5, 5), dtype=np.float32), angle=20,
                                     order=order)
