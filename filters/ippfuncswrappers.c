                        ...
                        float ippBorderValue)
{
    ...
    Ipp8u* pSrc1 = NULL, *pDst1 = NULL;     // Pointers to source/destination
                                            // images
    ...
    check_sts( status = ippiFilterGaussianInit(roiSize,
                                               ...
                                               pSpec,
                                               pBuffer));
    check_sts( status = ippiFilterGaussianBorder_8u_C1R(pSrc1,
                                                        srcStep,
                                                        pDst1,
                                                        dstStep,
                                                        roiSize,
                           ...
                           float ippBorderValue)
{
    ...
    check_sts( status = ippiFilterGaussianInit(roiSize,
                                               ...
                                               pSpec,
                                               pBuffer) );
    check_sts( status = ippiFilterGaussianBorder_8u_C3R(pSrc1,
                                                        srcStep,
                                                        pDst1,
                                                        dstStep,
                                                        roiSize,
...
{
    IppStatus status = ippStsNoErr;
    Ipp16u* pSrc1 = NULL, *pDst1 = NULL;    // Pointers to source/destination 
                                            // images
    int srcStep = stepSize, dstStep = stepSize;     // Steps, in bytes, through 
                                                    // the source/destination 
                                                    // images
    ...
    check_sts( status = ippiFilterGaussianInit(roiSize,
                                               ...
                                               pSpec,
                                               pBuffer));
    check_sts( status = ippiFilterGaussianBorder_16u_C1R(pSrc1,
                                                         srcStep,
                                                         pDst1,
                                                         dstStep,
                                                         roiSize,
...
{
    IppStatus status = ippStsNoErr;
    Ipp16s* pSrc1 = NULL, *pDst1 = NULL;     // Pointers to source/destination 
                                             // images
    int srcStep = stepSize, dstStep = stepSize;    // Steps, in bytes, through
                                                   // the source/destination
                                                   // images
    ...
    check_sts( status = ippiFilterGaussianInit(roiSize,
                                               ...
                                               pSpec,
                                               pBuffer));
    check_sts( status = ippiFilterGaussianBorder_16s_C1R(pSrc1,
                                                         srcStep,
                                                         pDst1,
                                                         dstStep,
                                                         roiSize,
                            ...
                            float ippBorderValue)
{
    ...
    check_sts( status = ippiFilterGaussianInit(roiSize,
                                               ...
                                               pSpec,
                                               pBuffer));
    check_sts( status = ippiFilterGaussianBorder_16u_C3R(pSrc1,
                                                         srcStep,
                                                         pDst1,
                                                         dstStep,
                                                         roiSize,
...
{
    IppStatus status = ippStsNoErr;
    Ipp32f* pSrc1 = NULL, *pDst1 = NULL;     // Pointers to source/destination 
                                             // images
    int srcStep = stepSize, dstStep = stepSize;     // Steps, in bytes, through
                                                    // the source/destination 
                                                    // images
    ...
    check_sts( status = ippiFilterGaussianInit(roiSize,
                                               ...
                                               pSpec,
                                               pBuffer) );
    check_sts( status = ippiFilterGaussianBorder_32f_C1R(pSrc1,
                                                         srcStep,
                                                         pDst1,
                                                         dstStep,
                                                         roiSize,
                             ...
                             float ippBorderValue)
{
    ...
    check_sts( status = ippiFilterGaussianInit(roiSize,
                                               ...
                                               pSpec,
                                               pBuffer) );
    check_sts( status = ippiFilterGaussianBorder_32f_C3R(pSrc1,
                                                         srcStep,
                                                         pDst1,
                                                         dstStep,
                                                         roiSize,
