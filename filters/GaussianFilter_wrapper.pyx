...
def gaussian(cnp.ndarray image,
             float sigma=1.0,
             output=None,
             mode='nearest',
             cval=0,
    ...
    if(numChannels == 1):
        if(destination.dtype == np.uint8):
            ...
            # needed more correct way. Warning: conversion from
            ...
            destination = np.asarray(destination,
                                     dtype=destination_actual_dtype)
    elif(numChannels == 3):
        if(destination.dtype == np.uint8):
            # If dtype is set, array is copied only if dtype does not match
            image = np.asarray(image, dtype=destination.dtype)
            ...
            cyimage = <void*> cnp.PyArray_DATA(image)
            cydestination = <void*> cnp.PyArray_DATA(destination)
            GaussianFilterUINT8RGB(cyimage,
                                   cydestination,
                                   img_width,
                                   img_height,
                                   numChannels,
            ...
            cyimage = <void*> cnp.PyArray_DATA(image)
            cydestination = <void*> cnp.PyArray_DATA(destination)
            GaussianFilterINT16RGB(cyimage,
                                   cydestination,
                                   img_width,
                                   img_height,
                                   numChannels,
            ...
            cyimage = <void*> cnp.PyArray_DATA(image)
            cydestination = <void*> cnp.PyArray_DATA(destination)
            GaussianFilterUINT16RGB(cyimage,
                                    cydestination,
                                    img_width,
                                    img_height,
                                    numChannels,
            ...
            cyimage = <void*> cnp.PyArray_DATA(image)
            cydestination = <void*> cnp.PyArray_DATA(destination)
            GaussianFilterFLOAT32RGB(cyimage,
                                     cydestination,
                                     img_width,
                                     img_height,
                                     numChannels,
